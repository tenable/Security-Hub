...
def test_processor_asset_trim(asset_aws, config):
    ...
    responses.post('https://cloud.tenable.com/assets/export',
                   json={'export_uuid': id}
                   )
    responses.get(f'https://cloud.tenable.com/assets/export/{id}/status',
                  json={
                    'status': 'FINISHED',
                    'chunks_available': [1]
                  })
    responses.get(f'https://cloud.tenable.com/assets/export/{id}/chunks/1',
                  json=[asset_aws]
                  )
    p = Processor(config)
...
def test_processor_ingest_warn(asset_local, finding, config, caplog):
    ...
    responses.post('https://cloud.tenable.com/vulns/export',
                   json={'export_uuid': id}
                   )
    responses.get(f'https://cloud.tenable.com/vulns/export/{id}/status',
                  json={
                    'status': 'FINISHED',
                    'chunks_available': [1]
                  })
    responses.get(f'https://cloud.tenable.com/vulns/export/{id}/chunks/1',
                  json=[finding]
                  )
    p = Processor(config)
    with caplog.at_level(logging.WARNING, logger='Tenb2SecHub'):
        with pytest.raises(ParamValidationError,
            ...
            p.ingest()
    msg = (f'Asset {asset_local["id"]} finding 106875 failed transformation '
